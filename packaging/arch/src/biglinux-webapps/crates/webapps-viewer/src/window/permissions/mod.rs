...
pub(super) fn prompt_permission<F: FnOnce(bool) + 'static>(
    ...
    // Custom appearances (Destructive deny + Suggested allow). Build via the
    // confirm helper with cancel-id "deny" so the shared adapter owns layout,
    // then override the deny appearance after the fact.
    let dialog = dialogs::confirm_dialog_with_cancel_id(
        &title,
        &body,
        "deny",
        &gettext("Deny"),
